...
//! `object_store`'s `LocalFileSystem`. It maps the object-store surface — including conditional
//! writes (`Create` / `Update` with ETags) — onto Loami's [`StorageProvider`] contract, and passes
//! the shared [`conformance`](loami_storage::conformance) suite.
//!
...
//! (compare-and-swap by ETag). This provider emulates `Update` by serializing writes with an
//! in-process lock, reading the current ETag, and overwriting only if it matches. That is correct
//! under Loami's single-writer model (one process owns a given store). It does **not** guard against
//! a second OS process writing the same directory concurrently; cross-process CAS via OS file locks
